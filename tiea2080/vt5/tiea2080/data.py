...
from flask import current_app as app, g, json
...
def alusta_tietokanta(**kwargs):
    ...
    with app.open_resource(kwargs['schema']) as f:
...
def avaa_yhteys(**kwargs):
    ...
    """
    kwargs.setdefault("database", app.config['DATABASE'])
...
class Data():
    ...
    def etsi_joukkue(self, **kwargs):
        """
        Etsii joukkuetta kannasta.

        :param id: Etsi joukkuetta IDn perusteella
        :type id: Int
        :param nimi: Etsi nimen perusteella.
        :param sarja: Jos nimi on määritelty, etsi vain tästä sarjasta.
        :param kilpailu: Rajaa etsiminen tähän kilpailuun.
        """
    ...
    def paivita_joukkue(self, joukkue):
        if type(joukkue['jasenet']) == "list":
            joukkue['jasenet'] = json.dumps(joukkue['jasenet'])
